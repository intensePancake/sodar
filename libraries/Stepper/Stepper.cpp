...
Stepper::Stepper(int number_of_steps, int motor_pin_1, int motor_pin_2)
{
  ...
  this->step_angle = 360.0 / (float) number_of_steps; // degrees per step
  this->angle = 0;			// current motor angle
...
/*
  Sets the origin angle for use with angular methods;
*/
...
int Stepper::getNumSteps(float angle)
{
	int steps = angle / this->step_angle;
	// try to get as close as possible to the requested angle
	if(angle > 0) {
		if(steps * this->step_angle + this->step_angle / 2 < angle) {
			steps += 1; // add an extra step to get the angle closer
		}
	} else {
		if(steps * this->step_angle - this->step_angle / 2 > angle) {
			steps -= 1;
